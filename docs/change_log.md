...
> 상세는 각 문서의 변경 이력 절과 `docs/freeze/` 의 요청서를 본다.
...
| 2026-08-21 | 공통 지침 | **v1.3** | §8.1 Conv 경계 규칙 Freeze (Padding=1 / Cross-Correlation / flip 없음) | `docs/freeze/D3_B_to_A_CNN_Convolution_Freeze_Request.md` | **A 승인 · B 확인 대기** |
| 2026-08-21 | 공통 지침 | v1.3 | §7.4 Tensor Memory Order = CHW | `docs/freeze/D3_FREEZE_REQUEST_A_001.md` 1 | A · **B/C 대기** |
| 2026-08-21 | 공통 지침 | v1.3 | §7.3 Event Tensor 전달 = Direct Handshake (`ext_*`) | 동 5 | A · **C 대기** |
...
| 2026-08-21 | 공통 지침 | **v1.4** | **§20.1 신규 — AXI Register Bit Field 전체 표 + Base `0x4000_0000`** | `docs/freeze/D3_FREEZE_REQUEST_A_002.md` | A · **C 대기** |
| 2026-08-21 | 공통 지침 | v1.4 | §20.1 `CTRL.INPUT_SRC` 신규 (0=PS/AXI 적재, 1=C 직결) | 동 §2.1 | A · **C 대기** |
...
| 2026-08-21 | interface_contract | **v0.3** | §10 신규 — AXI Register 계약 / C 연결 방법 / PS 폴링 규칙 | Phase 2 | A · **C 대기** |

| 2026-08-21 | 공통 지침 | **v1.5** | **기구 변경: Pan/Tilt 1개 → 2개 분리.** §15.0 / §15.2 신규 (PT#2 좌표 변환), §17 SAFE_LIMIT2 필수화, §20/§20.1 에 `0x48`~`0x54` 추가 | `docs/freeze/D3_FREEZE_REQUEST_A_002.md` rev.2 | A · **C 대기** |
| 2026-08-21 | 공통 지침 | v1.5 | §15.2 `PAN2_CMD = f(error_x)` 금지 명문화 (레이저가 오차를 따라가는 오구현 방지) | 동 §2.13 | A · **C 대기** |
...
| 2026-08-21 | interface_contract | **v0.4** | §11 신규 — Pan/Tilt 2 헤드 계약 | 동 | A · **C 대기** |

| 2026-08-21 | 역할 분담 | **v1.6** | §17 A Phase 3 완료 반영 (PS 소프트웨어 / C 통합 사전검증). 역할 경계 변경 없음 | Phase 3 | — |
| 2026-08-21 | (규격 변경 아님) | — | **A Phase 3**: `sw/` PS 드라이버·시험프로그램 추가, 호스트 목으로 45 check 검증, Vitis ELF 빌드. `rtl/integration/c_module_stub.v` 자리표시자로 C 통합 타이밍 사전측정 (LUT 1441→1785, WNS +0.782→**+1.121 ns**, 100MHz MET) | `docs/PHASE3_PS_SOFTWARE_REPORT.md` | — |
