...
> **2026-08-30 구현 상태 주의:** 이 문서는 변하지 않는 공통 계약의 최상위 정본이다.
> 본문의 `미착수`, `TBD`, 과거 진행률 문장은 작성 당시 이력으로 읽고, 실제 구현·Version·
> 검증 상태는 `PROJECT_STATUS.md`와 `integration_manifest.md`를 따른다.
